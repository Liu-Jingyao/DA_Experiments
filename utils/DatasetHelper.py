...
class DatasetHelper:
    ...
    def tokenize(self, my_tokenizer):
        dataset = self.dataset_dict.map(lambda batch: my_tokenizer(batch[self.dataset_config['text_field']], truncation=True)
                              , batched=True, batch_size=self.map_batch_size,
                              load_from_cache_file=False)
    ...
    def text_augmentation(self, text_augmentations, my_logger):
        ...
        if self.current_text_augmentation_flag in self.train_dataset.column_names:
            ...
            dataset_list = [self.train_dataset]
    ...
    def feature_augmentation(self, feature_augmentations, my_logger, my_tokenizer):
        ...
        if self.current_feature_augmentation_flag:
            self.current_feature_augmentation_flags[self.current_feature_augmentation_flag] = feature_augmentations[0]['prob']
        else:
